...
DATA_DIR = '../../data/nature/data-html2/' # you may have to create this
...
while True:
    ...
    def processEntry(entry):
        ...
        doi = entry['id']
        log_out.append('Record index: %d, DOI: %s' % (record_index, doi))
        ...
        if ext == 'pdf' or ext == 'html':
            log_out.append('Detected error, deleting..')
            os.remove(join(DATA_DIR, article_fname))

        return log_out

    num_cores = multiprocessing.cpu_count()
    results = Parallel(n_jobs=num_cores)(delayed(processEntry)(entry) for entry in entries)
